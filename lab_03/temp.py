...
def draw_line_brez_smoth(canvas, ps, pf, fill):
    I = 100
    fill = get_rgb_intensivity(canvas, fill, bg_color, I)
    dx = pf[0] - ps[0]
    dy = pf[1] - ps[1]
    sx = sign(dx)
    sy = sign(dy)
...
def draw_line_vu(canvas, ps, pf, fill):
    ...
    I = 100
    stairs = []
    fills = get_rgb_intensivity(canvas, fill, bg_color, I)
    if x1 == x2 and y1 == y2:
...
test_funcs = (cda_test, test_brez_float, test_brez_int, test_brez_smooth, test_vu)
